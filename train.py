...
class KWSDataModule(LightningDataModule):
    ...
    def collate_fn(self, batch):
        ...
        # print(mels.size(), "before")
        mels_arr = rearrange(mels, 'b c h (p w) -> b p (c h w)', p=self.patch_num)
        # print(mels_arr.size(), "after")
        # wavs = torch.stack(wavs)

        return mels_arr, labels
...
if __name__ == "__main__":
    ...
    datamodule = KWSDataModule(batch_size=args.batch_size, num_workers=args.num_workers,
                               ...
                               win_length=args.win_length, hop_length=args.hop_length, patch_num = 8,
    ...
    model = LitTransformer(num_classes=len(CLASSES), lr=0.001, epochs=30, 
